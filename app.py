...
with col1:
    st.metric("Total Business Clients", f"{kpi['total_business_clients']:,.0f}")
...
with col5:
    st.metric("High Value Clients", f"{kpi['high_value_clients']:,.0f}")
...
Avg Revenue: **${highlight['highest_segment_revenue']:,.2f}**
...
st.caption("Which client groups, regions, and contact strategies drive campaign conversion?")
...
st.dataframe(
    executive_action_df[display_cols],
